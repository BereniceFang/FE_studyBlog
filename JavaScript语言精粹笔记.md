...
* ==
  * === 和 !==, 与 == 和!= 的区别: == 和 != 会对比较的运算数进行强制类型转换
* with
  * with用来快捷的访问对象的属性, 但是结果有时不可预料
* eval
  * 作用: 传递一个字符串给JavaScript编译器, 并且执行其结果
  * 降低程序的安全性, 降低语言的性能
  * Function构造器是eval的另一种形式,也应避免使用
  * setTimeout和setInterval函数能接受字符串参数或函数参数, 如果是字符串参数会想eval一样处理, 也应避免
* continue
  * 可以跳到循环的顶部
  * 影响性能
* switch
  * 除非明确中断流程, 否则会穿越到下一个case条件
* 缺少块的语句
  * if while do for 都可以接受一个括在花括号中的代码块, 也可以接受单行语句, 但是容易引发问题, 所以建议用{}包住代码块
* ++ --
  * 可能会使代码变得不够严谨, 代码变得拥挤复杂和隐晦
* 位运算符
  * 没有整数类型所以要先转化为整数执行位运算之后再转换回去
* function语句对比function表达式
  * `var foo = function foo() {}`更好的体现了函数就是数值
  * function语句在解析时会被提升
```js
(function(){
  //立即执行匿名函数的方法
...
* 类型的包装对象
  ...
  * 尽量避免`new Object`和`new Array`, 使用{}和[]代替
* new
  * new会创建一个继承于其运算数的原型的新对象, 把新创建的对象绑定给this
  * 忘记用new的话会得到一个普通的函数调用, this会被绑定到全局, 容易污染全局变量
* void
  * void 是一个运算符, 接受一个运算数并返回undefined, 目前看来没用
...
* JSON对象容纳"名值"对的无序集合
  * 名字可以是任何字符串, 值可以是任何类型的JSON值
  * 可以无限层嵌套
* JSON数组是一个值的有序序列
  * 值可以是任何类型的JSON值
* JSON字符串被包围在一对双引号之间, \用于转义
  * JSON可以嵌入html的<script></script>标签中
* JSON数字与js数字相似, 首位不允许为0
